...
The installed standalone LID skills contain no discoverable version manifest; the conventions version remains unknown pending verification.

## LID Tooling

- **Coherence check**: `npm run check:intent` — validates spec references and requirement links to tests and implementation. Run `npm test`, `npm run test:e2e`, and `npm run build` separately for runtime and build verification.

## Commit Messages

Use Conventional Commits for every commit, for example `feat: add camera scanning`, `fix: preserve cached palettes`, or `docs: update setup instructions`.
